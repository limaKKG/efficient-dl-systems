...
def run_epoch(model, train_loader, val_loader, criterion, optimizer, profiler: Profile | None = None) -> tp.Tuple[float, float]:
...
def main():
    ...
    with Profile(model, schedule=schedule) as prof:
        run_epoch(model, train_loader, val_loader, criterion, optimizer, profiler=prof)
